...
function TimelineMap({ mr }) {
  return (
    <div className="tlmap">
      <div className="tlmaphd">{mr.mapName} <b>{mr.h}-{mr.a}</b></div>
      <div className="tlrow">
        {mr.rounds.map(rd => {
            ...
            <table>
              ...
              <tbody>
                {rows.map(({ pl, b }) => {
                  ...
                  return (
                    <tr key={pl.name}>
                      <td className="pl">{pl.name === mvpName && <span className="mvpstar">★</span>} {pl.name}<span className="agtag"> {ag}</span></td>
                      <td className={`rat ${ratCls}`}>{b.rating.toFixed(2)}</td>
                      <td className="acs">{b.acsFinal}</td><td>{b.k}</td><td>{b.d}</td><td>{b.a}</td>
